...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const { branding, products, cartCount, setIsCartOpen, searchQuery, setSearchQuery, selectedCategory, setSelectedCategory } = useStore();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isSearchExpanded, setIsSearchExpanded] = useState(false);

  // Derive unique categories from active store inventory
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <nav className="hidden lg:flex items-center space-x-7 text-xs font-semibold tracking-wider uppercase text-gray-600 ml-2">
                <button
                  ...
                  className={`transition-colors hover:text-black py-1 ${
                    currentView === 'shop' && selectedCategory === 'All' ? 'text-black border-b-2 border-[#FACC15] font-bold' : ''
                  }`}
                >
                  Home
                </button>
                  ...
                  <button
                    ...
                    className={`transition-colors hover:text-black py-1 ${
                      selectedCategory.toLowerCase() === cat.toLowerCase()
                        ? 'text-black border-b-2 border-[#FACC15] font-bold'
                        : ''
                    }`}
                  >
                    {cat}
        ...
        {/* Mobile Drawer Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden fixed inset-x-0 top-[var(--navbar-height,90px)] bg-white border-b border-gray-200 shadow-2xl p-6 z-50 animate-in slide-in-from-top duration-200">
